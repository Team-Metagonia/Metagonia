        ...
        if(bDrawLines)
        {
            ...
            if(fracturedChunk.ListAdjacentChunks.Count > 0)
            {
                Handles.DotHandleCap(
                    0, 
                    fracturedChunk.transform.position, 
                    Quaternion.identity, 
                    HandleUtility.GetHandleSize(fracturedChunk.transform.position) * 0.03f, 
                    EventType.Repaint
                );
            }
            ...
            foreach(FracturedChunk.AdjacencyInfo chunkAdjacency in fracturedChunk.ListAdjacentChunks)
            {
                if(chunkAdjacency.chunk)
                {
                    Handles.DotHandleCap(
                        0, 
                        chunkAdjacency.chunk.transform.position, 
                        Quaternion.identity, 
                        HandleUtility.GetHandleSize(chunkAdjacency.chunk.transform.position) * 0.03f, 
                        EventType.Repaint
                    );
